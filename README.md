...
| **Hosts** | Status de comunicação, portas ativas e cluster |
| **Enclosures** | Monitoramento global do chassi, inventário (Serial Number) e refrigeração (Fans) |
...
- Alerta de esgotamento de capacidade física no Pool (< 500 GB livres)
...
| `{$API.USER}` | Usuário da API |
| `{$API.PASS}` | Senha da API |
...
*Desenvolvido com foco em alta disponibilidade.*
